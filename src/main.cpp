...
unsigned int nTargetSpacing = 8 * 60;
unsigned int nStakeMinAge = 60.0 * 60.0 * 24.0 * 0.5;	// 12 hours
unsigned int nStakeMaxAge = 60.0 * 60.0 * 24.0 * 12.0;   // 12 days       
...
const string strMessageMagic = "mao Signed Message:\n";
...
const int DAILY_BLOCKCOUNT =  1440 / 8;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "mao", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "sparrows";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1490348127;
        }
        else
        {
            txNew.nTime    = 1490348123; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015.
        }
        txNew.vin.resize(1); 
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1490348129;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4250051;
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0x77191fcb805dbbcc6e696826e23cb13484e0871e661fce825ee82ac2a04111a2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xd3, 0x35, 0xf1 };
